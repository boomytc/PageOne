...
namespace db.bll
{
    public class bks_BookStock
    {
        ...
        public static void SaveData(db.dbEntities dc)
        {
            ...
            {
                //查询数据库中已经存在的数据
                DataTable tableExist = ef.ExecuteDataTable("select * from bks_BookStock");
                //获取要导入的数据
                ...
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    ...
                    DateTime stockDate = rui.typeHelper.toDateTime(table.Rows[i]["stockDate"]);
                    string userCode = rui.typeHelper.toString(table.Rows[i]["userCode"]);
                    string status = rui.typeHelper.toString(table.Rows[i]["status"]);
