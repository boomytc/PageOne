...
namespace db.bll
{
    ...
    public class bks_Book
    {
        ...
        //通过图书类型编号获取主键
        public static string getCodeByTypeCode(string bookTypeCode, db.dbEntities dc)
        {
            efHelper ef = new efHelper(ref dc);

            string sql = "select bookCode from dbo.bks_Book where bookTypeCode=@bookTypeCode ";
            object value = ef.ExecuteScalar(sql, new { bookTypeCode });
            return rui.typeHelper.toString(value);
        }
        //通过rowID获取图书类型编号
        public static string getTypeByRowID(string rowID, db.dbEntities dc)
        {
            efHelper ef = new efHelper(ref dc);

            string sql = "select bookTypeCode from dbo.bks_Book where rowID=@rowID ";
            object value = ef.ExecuteScalar(sql, new { rowID });
            return rui.typeHelper.toString(value);
        }
        //通过类型编号获取图书信息
        public static DataTable GetBooksByType(string bookType, db.dbEntities dc)
        {
            efHelper ef = new efHelper(ref dc);

            string sql = "select * from dbo.bks_Book where bookTypeCode=@bookTypeCode ";
            DataTable books = ef.ExecuteDataTable(sql, new { bookTypeCode = bookType });
            return books;
        }
        //获取实体
        ...
        public static void SaveData(db.dbEntities dc)
        {
            ...
            {
                //查询数据库中已经存在的数据
                DataTable tableExist = ef.ExecuteDataTable("select * from bks_Book");
                //获取要导入的数据
                ...
                for (int i = 0; i < table.Rows.Count; i++)
                {
                    ...
                    string bookName = rui.typeHelper.toString(table.Rows[i]["图书名称"]);
                    string isbnNO = rui.typeHelper.toString(table.Rows[i]["isbnNO"]);
                    decimal price = rui.typeHelper.toDecimal(table.Rows[i]["价格"]);
                    string author = rui.typeHelper.toString(table.Rows[i]["作者"]);
                    DateTime pressdate = rui.typeHelper.toDateTime(table.Rows[i]["出版日期"]);
                    string release = rui.typeHelper.toString(table.Rows[i]["release"]);
                    string issell = rui.typeHelper.toString(table.Rows[i]["isSell"]);
                    int stocksum = rui.typeHelper.toInt(table.Rows[i]["stockSum"]);
                    int sellsum = rui.typeHelper.toInt(table.Rows[i]["sellSum"]);
                    string remark = rui.typeHelper.toString(table.Rows[i]["remark"]);
                    string presscode = rui.typeHelper.toString(table.Rows[i]["出版社"]);
                    string booktype = rui.typeHelper.toString(table.Rows[i]["图书类别"]);


                    //1、如果主键不存在,则插入一条数据
                    if (tableExist.Select("bookCode='" + bookCode + "'").Length == 0)
                    {
                        db.bks_Book entry = new db.bks_Book();
                        entry.bookCode = bookCode;
                        entry.bookName = bookName;
                        entry.isbnNO = isbnNO;
                        entry.price = price;
                        entry.authorName = author;
                        entry.pressDate = pressdate;
                        entry.release = release;
                        entry.isSell = issell;
                        entry.stockSum = stocksum;
                        entry.sellSum = sellsum;
                        entry.remark = remark;
                        entry.pressCode = presscode;
                        entry.bookTypeCode = booktype;
                        entry.rowID = ef.newGuid();
                        dc.bks_Book.Add(entry);
                    }
                }
