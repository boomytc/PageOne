...
namespace db.bll
{
    ...
    public class bks_bookStockDetail
    {
        ...
        public static void SaveData(db.dbEntities dc)
        {
            ...
            {
                //查询数据库中已经存在的数据
                DataTable tableExist = ef.ExecuteDataTable("select * from bks_BookStockDetail");
                //获取要导入的数据
                ...
                {
                    //获取Excel导入的每一行数据
                    string stockCode = rui.typeHelper.toString(table.Rows[i]["stockCode"]);
                    int detailNo = rui.typeHelper.toInt(table.Rows[i]["detailNo"]);
                    string bookCode = rui.typeHelper.toString(table.Rows[i]["bookCode"]);
                    int quantity = rui.typeHelper.toInt(table.Rows[i]["quantity"]);
                    decimal price = rui.typeHelper.toDecimal(table.Rows[i]["price"]);


                    //1、如果主键不存在,则插入一条数据
                    if (tableExist.Select("stockCode='" + stockCode + "'").Length == 0)
                    {
                        db.bks_BookStockDetail entry = new db.bks_BookStockDetail();
                        entry.stockCode = stockCode;
                        entry.detailNo = detailNo;
                        entry.bookCode = bookCode;
